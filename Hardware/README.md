...
今回のファームウェアでは以下のようなピン配置にしています。ソースコードの書き換えで条件付きで変更できます。</br>
...
しかし、とりあえず、今回は大丈夫だと個人的には思っています。( **ダメだったらごめんなさい** )</br></br>
...
家庭用テレビ等で PicoDVI の映像出力を HDMI入力端子に繋いでも映らない、という事象があるそうです。その際は、Pico DVI Sock の側面にある 5V 端子に、5V or 3.3V を入れると、認識されて映るらしいです(私は未遭遇未検証)</br></br>

**ハードウェアとしては、単純に結線していくだけなので、作る難易度はかなり低いと思います。部品代もお安いですし。ぜひ挑戦してみてください。** </br></br>
